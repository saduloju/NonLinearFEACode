...
ElemU=zeros(1,ndf*nen);
%    ElemU=ElemU'
  for m=1:numel
    
     for ElemUnum= 1:4
       ElemU(ElemUnum*ndf-1) = d_applied((ix(m,ElemUnum)-1)*ndf+1);
       ElemU(ElemUnum*ndf) = d_applied((ix(m,ElemUnum)-1)*ndf+2);
     end
...
% for m=1:1
  ...
  ElemK = zeros(ndf*nen);
    %F_int=zeros(numnp*ndf,1);
  F_Assembly=zeros(numnp*ndf,1);
   F_int=zeros(ndf*nen,1);
...
%%  
  ...
  %obtain the internal force vector
  F_int = F_int + Bmat'*sigma *J;
 
     end
     
%% Internal Force Assembly   
    [F_Assembly] = Assembly_Force(ix,m,ndf,F_int);
